...
class Win32MidiPlayer():
    ...
    def openDevice(self,
                   deviceNumber=-1):  # device -1 refers to the default set in midi mapper, usually a good choice
        ...
        if rc != 0:
            raise Win32MidiException(
                'Error opening device, ' + self.midiOutOpenErrorCodes.get(rc,
                                                                          'Unknown error.'))
    ...
    def sendNote(self, pitch, duration=1.0, channel=1,
                 volume=60):  # duration in seconds
        midimsg = 0x90 + ((pitch) * 0x100) + (volume * 0x10000) + channel
        mm = c_int(midimsg)
        rc = self.winmm.midiOutShortMsg(self.hmidi, mm)
        if rc != 0:
            raise Win32MidiException(
                'Error opening device, ' + self.midiOutShortErrorCodes.get(rc,
                                                                           'Unknown error.'))
        ...
        if rc != 0:
            raise Win32MidiException(
                'Error sending event, ' + self.midiOutShortErrorCodes.get(rc,
                                                                          'Unknown error.'))
